...
if (isset($_SESSION['role'])) {
    ...
    if (isset($_POST['get_edit_item_div'])) {
        ...
        $sql_item = "SELECT i.item_id, i.item_name, i.item_cat, i.item_quantity, ip.price_max, ip.price_min, i.item_reference, i.barre_code, i.threshold FROM item AS i LEFT JOIN item_price AS ip ON i.item_id = ip.item_id
        ...
        $data = [$row_item['item_name'], $row_item['item_cat'], $row_item['item_reference'], $row_item['item_quantity'], $row_item['price_min'], $row_item['price_max'], $row_item['barre_code'], $row_item['threshold']];
        echo json_encode($data);
    } elseif (isset($_POST['AutoCompleteItem'])) {

        $input       = trim($_POST['Item']);
        $isBarcode   = preg_match('/^\d{8,20}$/', $input); // You can adjust length rules
        $textCherche = strtoupper($input);

        if ($isBarcode) {
            // Query by barcode
            $sql_item = "SELECT * FROM `item` as i, item_price as ip
        WHERE i.item_quantity > 0
        AND i.item_id = ip.item_id
        AND `barre_code` LIKE '$textCherche'
        AND ip.accreditation_date = (
            SELECT MAX(accreditation_date)
            FROM item_price AS ip2 WHERE ip2.item_id = i.item_id
        )";
        //echo $sql_item;
        } else {
            $textCherche = '%' . $textCherche . '%';
            // Query by item name
            $sql_item = "SELECT * FROM `item` as i, item_price as ip
        WHERE i.item_quantity > 0
        AND i.item_id = ip.item_id
        AND UPPER(`item_name`) LIKE '$textCherche'
        AND ip.accreditation_date = (
            SELECT MAX(accreditation_date)
            FROM item_price AS ip2 WHERE ip2.item_id = i.item_id
        )";
        ...
        $sql = "WITH
                    table1 AS (
                        SELECT
                        i.item_id, i.item_name, i.item_quantity,i.item_cat, ip.price_max, ip.price_min, i.threshold,
                    ...
                    table2 AS (
                        SELECT
                        table1.item_id, table1.item_name, table1.item_quantity, table1.item_cat, table1.price_max, table1.price_min, its.unit_price, table1.row_number_price, table1.threshold,
